...
class _SellEquippedStub:
    """首段商店决策:给 bench 0 号位挂 1 件装备再提案卖出(桩只服务
    本锁的可达性;真策略局带装卖出在默认批不可达,见对拍记录)。"""
    ...
    def decide_shop_screen(self, sess, cfg):  # noqa: ANN001
        ...
        if self.done or st.bench[0] is None:
            return []
        st.bench[0].equips = list(st.bench[0].equips or []) + ['铁皮护臂']
        self.done = True
        return [SellBench(bench_idx=0, reason='stub')]


def test_sell_equipped_bench_char_recycles_equip() -> None:
