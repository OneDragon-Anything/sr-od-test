"""息线族主题文件:R2 息线刷新门 floor 落码(P54/dd-026)+ P56/P57 可变现
息线下界 + T1 凑息卖语义重写 + 窗口 ADR-0516 重锚 + T-115 prep 凑息接线/
死金压库/Z1 卖回切除。

合并出处(2026-09-09 CUT2 合并批①,自 test_cw_realizable_interest_floor
整体并入,断言面原样迁入):
- R2 预算门息线 floor(出处 = P54-r2-interest-floor §③/§④ +
  dd-026-r2-interest-floor):floor 结构(门过帧的刷后投影金 ≥ 息线
  g* + Σ预留卡价 ρ,刷新通道永不掉满息档 + 至少一张命中卡可买不破线)/
  贴线拒刷(g* ≤ 金 < g*+ρ+c_eff 帧 ⇒ r1 过而 r2 关,不发射刷新,
  P40 R2 溢余段刷窗 n_max=0 的行为面)/ ρ 注册表派生(Σ预留卡价 =
  合格集最低费卡价 CHARACTERS 现读,禁字面量;可追过滤与 R1 装配侧
  同一)/ cap 语境(g* 随 cap_resolved 参数化,息律投资 cap=10 局息线
  100,防把 50 拍成域常数 = dd-026 备选 2 禁案的判别锁)。
- P56/T1/P57(出处 = docs/develop/currency_war/strategy/
  13_buy_face_design.md §2.2/§2.3/§3.2):P56 下界构造 s_reserve :=
  g* − Σ活期退金投影 + T1 三项语义重写(「已落地并可对拍」验收定义
  R2-N4 三断言 + R2-N1 发射约束)+ T1 短路径布尔门退役 + P57 双读法
  + 分键遥测四字段 R3-R5。
- 窗口判据 ADR-0516 重锚(塌缩带锚;P57 读法问题随 V̄ 退役消解)。
- T-115 规则②:凑息卖 prep 接线 + 死金压库买入 + Z1 卖回切除(静态
  排除红证/锁线宽集/在售未买覆盖/动态排除轮界 P78-3,ADR-0580)。
"""
