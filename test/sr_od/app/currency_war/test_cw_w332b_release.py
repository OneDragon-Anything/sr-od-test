...
- ④ spend_mode release 状态机:_economy_mode_for release→adaptive;
  _maybe_sell_for_interest release 档不卖息;
...
def test_economy_mode_maps_release_to_adaptive(monkeypatch) -> None:
    """spend_mode 消费侧(评分消费者):release → adaptive(中性,不强化守息)。"""
    from sr_od.application.currency_war import cw_evaluate
    monkeypatch.setattr(cw_evaluate, 'get_node_goal',
                        lambda *a, **k: NodeGoal(6, 'release'))
    st = _state()
    assert cw_evaluate._economy_mode_for(st) == 'adaptive'


def test_sell_for_interest_skips_release(monkeypatch) -> None:
    """spend_mode 消费侧(动作消费者):release 档不卖息凑档(同 allin/level)。"""
    ...
    # 对照:adaptive 档同帧照卖(证明跳过来自 release 档而非别的门)
    monkeypatch.setattr(cw_plan, 'get_node_goal',
