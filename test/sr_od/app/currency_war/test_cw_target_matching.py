...
def test_equip_allocation_carry_first() -> None:
    ...
    alloc = equip_allocation(lt, dep, ['火力风暴潮', '高周波电锯', '火力风暴潮', '蓄能帆'])
    # 姬子(carry)先拿:风暴潮+风暴潮+电锯(容量3内按 key 顺序)
    jz = [e for c, e in alloc if c == '姬子·启行']
    assert jz == ['火力风暴潮', '火力风暴潮', '高周波电锯'], f"carry 按序拿 key,得 {jz}"
...
def test_equip_allocation_capacity_and_fallback() -> None:
    """容量扣减(已穿 3 = 满)与 comp=None 通用兜底(前排先)。"""
    from types import SimpleNamespace
    ...
    # comp=None → 全通用:瓦尔特(前排先)全拿
    alloc = equip_allocation(None, dep, ['永动机', '蓄能帆'])
    assert alloc == [('瓦尔特', '永动机'), ('瓦尔特', '蓄能帆')]
    # 容量:瓦尔特已穿满 3 → 让位给符玄
    occ = {('front', 1): ['a', 'b', 'c']}
