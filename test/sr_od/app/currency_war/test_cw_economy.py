...
def _seed_three_stream(rec: Path) -> None:
    ...
    spend_ledger 行造为 legacy 形态(无 gold_close 字段,W505 金面收口前的
    行)——关店金走冲突行回退 join,正是本 fixture 要测的三流合账面;
    slot 期新行(字段在场)不回退的语义由 resolve_unit_gold_close 新锁锁。
    """
...
# ===== EXEC_FAIL_P3R1 修复锁(安灯 gold_close 数据源改 spend_ledger 单元行;
# 根因=冲突行「仅 mismatch 才写 + 无身份键」被同轮上一单元陈旧行 join 进来
# → 健康单元金差算 0 误停。fixture 形态取自局 run_20260901_180236 p3r1)=====

def _plan_p3r1_buy_lvup():
    """误停案第二单元真实 plan 形态:BuyCard(星期日 3 金)+ LevelUp(4 金),
    开店金 51,计划净 -7。"""
    return [
        {'__type__': 'BuyCard', 'card': {'name': '星期日', 'cost': 3}},
        {'__type__': 'LevelUp', 'cost': 4},
    ]


def _stale_conflict_row() -> dict:
    """同轮第一单元 86 秒前的 gold_delta 冲突行(误停案的陈旧行 new=51)。"""
    return {'ts': '2026-09-01T19:09:57', 'field': 'gold_delta', 'old': 48,
            'new': 51, 'verdict': '留证-动作账vs读数不等',
            'source': 'shop_spend_audit', 'plane': 3, 'round_num': 1,
            'spend': 22, 'run_id': 'run_20260901_180236'}


def test_resolve_gold_close_prefers_unit_row_no_false_stop(tmp_path: Path):
    """陈旧行场景不误停:单元行自带 gold_close=44(每单元必写、身份键完整)
    时,同轮陈旧冲突行 new=51 不再被吃进来 → effective,不停。"""
    unit_row = {'run_id': 'r', 'plane': 3, 'round_num': 1, 'unit_seq': 1,
                'gold_close': 44, 'gold_close_trusted': True}
    gc = query.resolve_unit_gold_close(
        unit_row, [_stale_conflict_row()], 3, 1, '2026-09-01T19:11:23')
    assert gc == 44
    from sr_od.application.currency_war.run_state import exec_fail_should_stop
    assert exec_fail_should_stop(_plan_p3r1_buy_lvup(), 51, gc) is False
    r = query.classify_spend_unit(_plan_p3r1_buy_lvup(), 51, gc)
    assert (r['verdict'], r['actual_delta'], r['gap']) == ('effective', -7, 0)


def test_resolve_gold_close_true_failure_still_stops(tmp_path: Path):
    """真失败场景仍停:单元行 gold_close=51(金确实没动,计划花费 7)→
    not_effective → 停——修数据源不收窄安灯停线面。"""
    unit_row = {'run_id': 'r', 'plane': 3, 'round_num': 1, 'unit_seq': 1,
                'gold_close': 51, 'gold_close_trusted': True}
    gc = query.resolve_unit_gold_close(
        unit_row, [_stale_conflict_row()], 3, 1, '2026-09-01T19:11:23')
    assert gc == 51
    from sr_od.application.currency_war.run_state import exec_fail_should_stop
    assert exec_fail_should_stop(_plan_p3r1_buy_lvup(), 51, gc) is True


def test_resolve_gold_close_read_fail_no_stale_fallback():
    """新行读失败(gold_close=None,trusted=False)不得回退陈旧冲突行——
    回退 = 把误停根因的 join 面从主路径挪进读失败路径。None 进分类器
    记 unknown 不猜,不停。"""
    unit_row = {'run_id': 'r', 'plane': 3, 'round_num': 1, 'unit_seq': 1,
...
def test_resolve_gold_close_legacy_row_fallback():
    """历史旧行(行内无 gold_close 字段)保留冲突行回退——存量 replay 可读,
    行为不因修复漂移。"""
    legacy_row = {'run_id': 'r', 'plane': 3, 'round_num': 1, 'unit_seq': 1}
    gc = query.resolve_unit_gold_close(
        legacy_row, [_stale_conflict_row()], 3, 1, '2026-09-01T19:11:23')
    assert gc == 51


def test_spend_unit_open_seq_round_keyed_not_run_entry_reset():
...
# (W829 支出门锁段已随 spend_gate 开关族删除——旧方案清退批,清查
#  报告 OLD_MIX_AUDIT §1.3;decision_v2/spend_gate.py 模块、约束名与
#  v3_sg_block 遥测键同批删。)
...
def test_reconcile_allocator_jurisdiction_records_only(monkeypatch) -> None:
    """死亡域帧:授权未兑现只记录交分配器(action='allocator'),不降级。
    (原用兑现链 D2 入口帧臂显式开构造死亡域;D2 臂已随 realization_
    chain 开关族删除——旧方案清退批,清查报告 OLD_MIX_AUDIT §1.3,
    改 stub 终止分支谓词 terminal_release 构造同辖域,断言面不变。)"""
    st = _spend_receipt_st(node='battle', gold=61, hp=20, round_num=1, bench=_BENCH_WAITING)
    import sr_od.application.currency_war.decision.decision_v2.discipline as _sr_disc
    monkeypatch.setattr(_sr_disc, 'terminal_release',
                        lambda state, session, registry: True)
    sess = _spend_receipt_sess(_spend_receipt_Posture(level_up=True, tag='升级'), st)
    assert alloc_domain(st, sess, _spend_receipt_DEFAULT_REGISTRY) is not None
    _spend_receipt_arbitrate([], st, sess, _spend_receipt_DEFAULT_REGISTRY)
    un = sess.v3_posture_unfulfilled
