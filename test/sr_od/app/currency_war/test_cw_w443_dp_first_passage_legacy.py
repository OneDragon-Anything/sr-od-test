"""W443 DP/first_passage 面单帧锁(legacy_baseline 桶)。

2026-09-03 拆分批:自 test_cw_w443_two_state_unification.py 拆出——被测对象
为 DP 两态/first_passage/损血标定面(随旧核伤害世界模型退役而消亡,打
legacy_baseline 标,默认快速集/全量 -m "not slow and not legacy_baseline"
跳过)。effective_hp_threshold 共享面(消费点/现值/P1 零漂移)保留原文件。
