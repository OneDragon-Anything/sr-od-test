...
def _c1_directed_spend_card(name: str, cost: int = 1) -> _c1_directed_spend_ShopCard:
    return _c1_directed_spend_ShopCard(name=name, faction='仙舟罗浮', cost=cost, x=0, star=1)


def _unit(slot: int, name: str = '件', front: bool = True) -> _c1_directed_spend_BenchChar:
    return _c1_directed_spend_BenchChar(slot=slot, char_id=name, faction='仙舟罗浮', star=1,
                     position_pref='front' if front else 'back')


def _c1_state(**kw) -> _c1_directed_spend_GameState:
    ...
    base.update(kw)
    return _c1_directed_spend_GameState(**base)


def _boss_session() -> _c1_directed_spend_StrategySession:
    sess = _c1_directed_spend_StrategySession()
...
def _cands(target: str, merge: bool = False) -> list[_c1_directed_spend_Candidate]:
    """六类候选各一:目标件买/非目标件买/升级/刷新/卖/上阵。"""
    return [
        _c1_directed_spend_Candidate(action=BuyCard(_c1_directed_spend_card(target), reason=''), tag='line_carry',
                  source='shop', merge=merge),
        _c1_directed_spend_Candidate(action=BuyCard(_c1_directed_spend_card('散件甲'), reason=''), tag='plugin',
                  source='shop'),
        _c1_directed_spend_Candidate(action=_c1_directed_spend_LevelUp(cost=4), tag='levelup', source='xp'),
        _c1_directed_spend_Candidate(action=_c1_directed_spend_RefreshShop(cost=2), tag='refresh', source='shop'),
        _c1_directed_spend_Candidate(action=SellBench(bench_idx=0, income=1, expect=''),
                  tag='for_gold', source='bench'),
        _c1_directed_spend_Candidate(action=DeployMove(bench_idx=1, to_row='front',
                                    faction='仙舟罗浮'),
...
def test_c1_scope_guards() -> None:
    ...
    assert c1_directed_active(
        _c1_state(round_num=9), _c1_directed_spend_StrategySession(), _REG_C1), \
        'node 缺读 r9 兜底也是 boss 窗统一口径的一部分,r9 帧仍辖'
    assert not c1_directed_active(
        _c1_state(round_num=5), _c1_directed_spend_StrategySession(), _REG_C1), \
...
def _bench_full_state(**kw) -> _c1_directed_spend_GameState:
...
def test_c1_narrows_zero_delta_spend() -> None:
    ...
    kept, flog = _c1_directed_spend_filter_candidates(_cands(tgt), st, _boss_session(), _REG_C1)
...
def test_c1_keeps_merge_and_directed_refresh() -> None:
    ...
    判据)。"""
    tgt = _target_name()
    st_room = _c1_state(shop=[_c1_directed_spend_card(tgt)])
    kept, _ = _c1_directed_spend_filter_candidates(_cands(tgt), st_room, _boss_session(),
                                _REG_C1)
    tags = {c.tag for c in kept}
    ...
    kept2, flog2 = _c1_directed_spend_filter_candidates(_cands(tgt, merge=True), st_merge_bench,
    ...
    kept3, _ = _c1_directed_spend_filter_candidates(_cands(tgt, merge=True), st_merge_board,
                                 _boss_session(), _REG_C1)
