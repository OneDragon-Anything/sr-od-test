...
leader 审后转锁(2026-08-24)。覆盖:污染红/历史 schema 红/全 live
绿/值相交绿/空不辖/无读数不辖。
"""
...
class TestDifficultyLiveContamination:
    def test_mixed_disjoint_sets_violation(self) -> None:
        ...
        assert any('污染' in v or '判废' in v for v in r.get('violation_list', [v for v in [r.get('note', '')] if v])) or r['violations'] >= 1
