...
# ==================== 不变量「全未识别=不定」回归锁 ====================
# (2026-09-09 合并批并入:原 test_cw_shop_refresh_unknown_invariant.py 全文件
#  迁入本尾段;观测自检框架设计 §2.5。背景:生产不变量比较的退化解——某局
#  5/5 牌全未识别时,刷前刷后同为 ['', '', '', '', ''],集合相等会被误读成
#  「刷新未生效」→ 假阳性停线。生产判据 :func:`refresh_effective` 任一侧含
#  未识别槽('')即返 None(cw_op_buy_cards.refresh_effective,单一源);写入端
#  落票只认 ``is False``(cw_shop_action_ops.py 刷新段 ledger.refresh_board_changed),
#  故该形态在生产不可达——本锁钉住防回流。真值表其余用例单一源 = 本文件
#  w514 段 test_refresh_effective_truth_table(单槽 ''/空列表彼处已有;本段只补
#  「全未识别 5/5」形态,勿重复铺开;refresh_effective 复用 w514 段模块级
#  import。字母占位桩禁用——2026-09-03 实证其随测试泄漏曾被误判实机停线)。)
