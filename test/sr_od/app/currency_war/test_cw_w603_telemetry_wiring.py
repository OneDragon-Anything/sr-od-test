"""W603 观测接线修复批:三件观测面接线的行为锁(零决策行为变更)。

① 披露键进 decisions 遥测行:血预算停手拒付计数(v3_blood_budget_rejects /
   v3_blood_budget_refresh_rejects)session 透传、末窗降格触发面
   (p1_downgrade_active,纯谓词现算)、经验账本(xp_expect_ledger,正式字段
   平铺)——接出点 = recorder 统一自 _CTX_MATCH_REF 自取,extra 通道不动。
② obs_conflicts.jsonl 补 run_id:唯一汇点 obs_conflict() 内部自取
   current_run_id 补键;历史行无键,读取端「有键才过滤」容忍。
③ 简报行 run_id 归属:简报在 start_run 之前读,此前空 run_id=行被丢 /
   旧 run_id=归属滞后(W576 组5.1);修法 = 局前 briefing 行缓冲,
