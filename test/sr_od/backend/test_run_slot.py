...
from sr_od.backend.backend_context import RunState
...
def test_context_query_status_delegates(slot, mock_ctx):
    """SrBackendContext.query_status 转发 run_slot._query_status。"""
    from sr_od.backend.backend_context import SrBackendContext
...
def test_context_stop_delegates(slot, mock_ctx):
    """SrBackendContext.stop 封装 run_slot._stop,无运行时返 {stopped:False, error}。"""
    from sr_od.backend.backend_context import SrBackendContext
