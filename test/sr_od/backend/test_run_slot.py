...
def test_op_path_uses_display_name_as_op_id(slot):
    ...
    _, fut = slot._start('mcp', op_factory=_make_op(OperationResult(success=True)),
                         display_name='sr_od.operations.enter_game.OpenAndEnterGame')
    fut.result(timeout=5)
    assert slot.op_id == 'sr_od.operations.enter_game.OpenAndEnterGame'
...
# ============ SrBackendContext 委托(单槽:start_run/query_status/stop 都走 run_slot) ============

def test_context_start_run_delegates(slot, mock_ctx):
    """SrBackendContext.start_run 转发 run_slot._start,返回 (ok, future)。"""
...
def test_context_query_status_delegates(slot, mock_ctx):
    """SrBackendContext.query_status 单槽转发(idle 可达)。"""
    from sr_od.backend.backend_context import SrBackendContext
...
def test_context_stop_delegates(slot, mock_ctx):
    """SrBackendContext.stop 单槽封装(无运行时返 {stopped:False, error})。"""
    from sr_od.backend.backend_context import SrBackendContext
