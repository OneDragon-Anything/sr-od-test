...
pytestmark = pytest.mark.skip(reason="ZZZ operation 空间专属（断言 hollow_zero/HollowRunner/DropResonium 等），SR 的 operations 不同（无 hollow_zero）——需 SR 专属重写再启用")

"""operation 扫描 / op_id 解析 / args 校验的单元测试。

测试使用 MagicMock 伪造 SrContext(扫描纯反射,不依赖真实游戏/模型)。
...
from sr_od.backend.operation_registry import (
    describe_operation,
    resolve_op_class,
    scan_operations,
    validate_args,
...
def test_scan_includes_known_ops(mock_ctx: MagicMock) -> None:
    ...
    assert 'sr_od.operations.enter_game.open_and_enter_game.OpenAndEnterGame' in op_ids
...
def test_scan_import_failure_tolerant(mock_ctx: MagicMock, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def flaky_import(name: str):  # noqa: ANN202
        if name == 'sr_od.operation.map_transport':
            raise RuntimeError('模拟 import 失败')
        return real_import(name)

    monkeypatch.setattr(
        'sr_od.backend.operation_registry.importlib.import_module', flaky_import
    )
    result = scan_operations(mock_ctx, refresh=True)
    # 失败被记录
...
def test_scan_op_info_fields(mock_ctx: MagicMock) -> None:
    ...
    assert transport.op_id == 'sr_od.operation.map_transport.MapTransport'
    assert transport.module == 'sr_od.operation.map_transport'
...
def test_resolve_known_op() -> None:
    """解析已知 operation 类。"""
    cls = resolve_op_class('sr_od.operations.enter_game.open_and_enter_game.OpenAndEnterGame')
...
def test_resolve_rejects_non_operation() -> None:
    ...
    with pytest.raises(ValueError):
        resolve_op_class('sr_od.application.charge_plan.charge_plan_config.ChargePlanItem')
...
def test_resolve_rejects_rexport() -> None:
    ...
    with pytest.raises(ValueError):
        resolve_op_class('sr_od.operations.enter_game.open_and_enter_game.OpenGame')
...
def test_validate_args_rejects_complex_dataclass(mock_ctx: MagicMock) -> None:
    ...
    from sr_od.operation.compendium.notorious_hunt import NotoriousHunt
    # 缺参 → 拒
...
def test_describe_operation_reflects_params(mock_ctx: MagicMock) -> None:
    ...
    info = describe_operation(mock_ctx, 'sr_od.operation.map_transport.MapTransport')
    assert info['op_id'] == 'sr_od.operation.map_transport.MapTransport'
...
def test_describe_operation_marks_complex_param(mock_ctx: MagicMock) -> None:
    ...
    info = describe_operation(
        mock_ctx, 'sr_od.operation.compendium.notorious_hunt.NotoriousHunt',
    )
